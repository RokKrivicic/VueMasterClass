...
describe('actions', () => {
  ...
  describe('FETCH_JOBS', () => {
    it('makes API request and store received jobs', async () => {
      ...
      expect(store.jobs).toEqual(['Job 1', 'Job']);
    });
  });
});
...
describe('getters', () => {
  ...
  describe('INCLUDE_JOB_BY_ORGANIZATION', () => {
    ...
    it('identifies if job is asociated with given organization', () => {
      const userStore = useUserStore();
  ...
  describe('INCLUDE_JOB_BY_JOB_TYPE', () => {
    ...
    it('identifies if job is asociated with given job types', () => {
      const userStore = useUserStore();
      userStore.selectedJobTypess = ['Full-time', 'Part-time'];
      const store = useJobsStore();
      const job = { jobType: 'test' };
